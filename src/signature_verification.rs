...
#[derive(Debug, Clone)]
pub struct SignatureVerificationConfig<F: PrimeField> {
    pub rsa_config: RSAConfig<F>,
}

pub const LIMB_BITS: usize = 64;

impl<F: PrimeField> SignatureVerificationConfig<F> {
    /// Construct a new [`SignatureVerificationConfig`].
    ///
    /// # Arguments
    /// * `range_config` - a configuration for [`RangeConfig`].
    /// * `public_key_bits` - the number of bits of RSA public key.
    /// # Return values
    /// Return a new [`SignatureVerificationConfig`].
