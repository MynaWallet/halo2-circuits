...
impl<F: BigPrimeField, T: RangeType> AssignedBigUint<F, T> {
    ...
    pub fn extend_limbs(&mut self, num_extend_limbs: usize, zero_value: AssignedValue<F>) -> Self {
...
/// `RefreshAux` は、`Muled`型の整数を`Fresh`型の整数にリフレッシュするための補助データを提供します。
#[derive(Debug, Clone)]
pub struct RefreshAux {
    limb_bits: usize,                // リンブのビット長
    num_limbs_l: usize,              // l整数のリンブの数
    num_limbs_r: usize,              // r整数のリンブの数
    increased_limbs_vec: Vec<usize>, // 拡張されたリンブ数のベクトル
}

impl RefreshAux {
    /// 新しい`RefreshAux`インスタンスを作成します。
    ///
    /// # 引数
    /// * `limb_bits` - リンブのビット長。
    /// * `num_limbs_l` - l整数のリンブの数。
    /// * `num_limbs_r` - r整数のリンブの数。
    ///
    /// a（b）が整数lとrの積である場合、`num_limbs_l` と `num_limbs_r` でそれぞれのリンブの長さを指定する必要があります。
    ///
    /// # 戻り値
    /// 新しい `RefreshAux` インスタンス。
    pub fn new(limb_bits: usize, num_limbs_l: usize, num_limbs_r: usize) -> Self {
        ...
        // l_max と r_max の積を計算し、結果を muled に格納します。
        ...
        // muled の各エントリに対して、リンブのビット数が limb_bits を超えている場合、
        // それを複数のリンブに分割します。
